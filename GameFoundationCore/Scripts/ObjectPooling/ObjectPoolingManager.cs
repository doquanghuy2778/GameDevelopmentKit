namespace GameDevelopmentKit.GameFoundationCore.ObjectPooling
{
    ...
    using IGameAssets = GameFoundationCore.AssetsManager.IGameAssets;
    ...
    public class ObjectPoolingManager
    {
        ...
        private readonly Dictionary<GameObject, ObjectPooling> prefabToObjectPooling     = new();
        private readonly Dictionary<GameObject, ObjectPooling> spawnedObjToObjectPooling = new();
        ...
        [Preserve]
        public ObjectPoolingManager(IGameAssets gameAssets)
        {
        ...
        public ObjectPooling CreatePool<T>(T prefab, int initialPoolSize, GameObject root) where T : Component { return this.CreatePool(prefab.gameObject, initialPoolSize, root); }

        public ObjectPooling CreatePool(GameObject prefab, int initialPoolSize, GameObject root)
        {
            if (prefab == null) return null;

            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool)) return pool;

            pool = new GameObject($"[Pool] {prefab.name}", typeof(ObjectPooling)).GetComponent<ObjectPooling>();

            pool.transform.SetParent(this.ChooseRoot(root).transform, false);
            this.prefabToObjectPooling.Add(prefab, pool);
        ...
        private GameObject ChooseRoot(GameObject root)
        {
            ...
            if (this.defaultRoot == null) this.defaultRoot = new() { name = "ObjectPoolingManager" };

            return this.defaultRoot;
        }

        public int CountPooled<T>(T prefab) where T : Component { return this.CountPooled(prefab); }

        public int CountPooled(GameObject prefab) { return this.prefabToObjectPooling.TryGetValue(prefab, out var pool) ? pool.pooledObjects.Count : 0; }

        public int CountSpawned<T>(T prefab) where T : Component { return this.CountSpawned(prefab); }

        public int CountSpawned(GameObject prefab) { return this.prefabToObjectPooling.TryGetValue(prefab, out var pool) ? pool.spawnedObjects.Count : 0; }

        public int CountAllPooled()
        {
            var count                                                     = 0;
            foreach (var pool in this.prefabToObjectPooling.Values) count += pool.pooledObjects.Count;
            return count;
        }
        ...
        public List<GameObject> GetPooled(GameObject prefab, List<GameObject> list, bool appendList)
        {
            ...
            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool)) list.AddRange(pool.pooledObjects);
            return list;
        }
        ...
        public List<T> GetPooled<T>(T prefab, List<T> list, bool appendList) where T : Component
        {
            if (list == null) list = new();
            if (!appendList) list.Clear();
            if (this.prefabToObjectPooling.TryGetValue(prefab.gameObject, out var pool))
            {
        ...
        public List<GameObject> GetSpawned(GameObject prefab, List<GameObject> list, bool appendList)
        {
            if (list == null) list = new();
            if (!appendList) list.Clear();
            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool)) list.AddRange(pool.spawnedObjects);
            return list;
        }
        ...
        public async UniTask<ObjectPooling> CreatePool(string prefabName, int initialPoolSize, GameObject root)
        {
        ...
        public GameObject Spawn(GameObject prefab, Transform parent, Vector3 position, Quaternion rotation)
        {
            ...
            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool))
            {
                var spawnedObj = pool.Spawn(parent, position, rotation);
                this.spawnedObjToObjectPooling.Add(spawnedObj, pool);
                return spawnedObj;
            }
        ...
        {
            if (this.spawnedObjToObjectPooling.TryGetValue(obj, out var pool))
            {
                pool.Recycle(obj);
                if (parent) obj.transform.SetParent(parent);
                this.spawnedObjToObjectPooling.Remove(obj);
            }
            else
            {
                throw new($"Can't recycle object, maybe you already recycled it!");
            }
        }
        ...
        {
            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool) && pool.spawnedObjects.Count > 0)
            {
        ...
        {
            this.tempList.AddRange(this.spawnedObjToObjectPooling.Keys);
            for (var i = 0; i < this.tempList.Count; ++i) this.Recycle(this.tempList[i]);
            this.tempList.Clear();
        }
        ...
        {
            if (prefab != null && this.prefabToObjectPooling.TryGetValue(prefab, out var pool)) pool.CleanUpPooled();
        }
        ...
        public void CleanUpAll(GameObject prefab)
        {
            ...
            this.prefabToObjectPooling.Remove(prefab);
        }
        ...
        {
            if (this.prefabToObjectPooling.TryGetValue(prefab, out var pool)) Object.Destroy(pool.gameObject);
        }
        #endregion
    }
}